 ...
 *
 * Assigned User matching: the "Assigned User" Excel column is matched
 * against the application's Full Name first (case-insensitive), falling
 * back to Username if no full-name match is found. This accommodates
 * real-world source data (CRM/ticketing exports) which typically records
 * a person's display name rather than their internal system username.
 */
@Service
...
public class ExcelImportServiceImpl implements ExcelImportService {
    ...
    /**
     * Resolves the "Assigned User" Excel value to an active application user.
     * Tries an exact full-name match first, then falls back to username,
     * both case-insensitive. Returns empty if neither matches or the
     * matched user is inactive.
     */
    private Optional<User> resolveAssignedUser(String assignedUserValue) {
        if (StringUtils.isBlank(assignedUserValue)) {
            return Optional.empty();
        }
        String trimmed = assignedUserValue.trim();
        Optional<User> byFullName = userRepository.findByFullNameIgnoreCase(trimmed);
        if (byFullName.isPresent()) {
            return byFullName.filter(User::isActive);
        }
        Optional<User> byUsername = userRepository.findByUsernameIgnoreCase(trimmed);
        return byUsername.filter(User::isActive);
    }
        ...
        try (InputStream is = file.getInputStream(); Workbook workbook = WorkbookFactory.create(is)) {
            ...
            for (int rowIdx = 1; rowIdx <= sheet.getLastRowNum(); rowIdx++) {
                ...
                if (assignedUserValue != null) {
                    Optional<User> assignedUserOpt = resolveAssignedUser(assignedUserValue);
                    if (!assignedUserOpt.isPresent()) {
                        errors.add("Assigned User '" + assignedUserValue + "' is not a known active application user.");
                    }
                }
    ...
    private Ticket applyRowToTicket(Map<String, String> data, StatusMaster defaultStatus, User uploader) {
        ...
        String assignedUserValue = StringUtils.trimToNull(data.get("Assigned User"));
        User assignedUser = assignedUserValue != null
                ? resolveAssignedUser(assignedUserValue).orElse(ticket.getAssignedUser())
